...
try {
    switch ($action) {
        ...
        case 'settings':
            $default_model = getenv('OPENAI_MODEL') ?: 'gpt-5.1-codex-max';
            $locked = true;
            $models = default_role_models();
            foreach ($models as $role => $_) {
                $models[$role] = $default_model;
            }
            json_response([
                'settings' => ['models' => $models, 'locked' => $locked],
        ...
        case 'update_settings':
            json_response(['error' => 'Model selection is locked in v3.'], 403);
            break;
        ...
        case 'debug_status':
            json_response(['enabled' => read_debug_enabled()]);
            break;
