...
4) Every task must have a Task Pack and Result Pack:
   - /tasks/<id>_taskpack.md
   - /tasks/<id>_result.md
5) Non-trivial decisions go to /tasks/ACTIVE.md under "Key Decisions".
...
C) Verify: run commands from Task Pack or /tasks/ACTIVE.md and record results
...
## Task Packs
- Active Task Pack: tasks/<id>_taskpack.md
- Result Pack: tasks/<id>_result.md
...
function task_pack_id(int $job_id, int $step_index): string
{
    return 'job_' . $job_id . '_step_' . $step_index;
}

function task_pack_path(int $job_id, int $step_index): string
{
    return 'tasks/' . task_pack_id($job_id, $step_index) . '_taskpack.md';
}

function result_pack_path(int $job_id, int $step_index): string
{
    return 'tasks/' . task_pack_id($job_id, $step_index) . '_result.md';
}

function format_pack_list_line(string $label, array $items): string
{
    if (!$items) {
        return '- ' . $label . ': none';
    }
...
function format_task_pack(string $task_id, array $step): string
{
    $title = trim((string) ($step['title'] ?? ''));
    if ($title === '') {
        $title = trim((string) ($step['goal'] ?? 'Task'));
    }
    $goal = trim((string) ($step['goal'] ?? ''));
    $in_scope = normalize_string_list($step['in_scope'] ?? []);
    $out_scope = normalize_string_list($step['out_of_scope'] ?? []);
    $acceptance = normalize_string_list($step['acceptance_criteria'] ?? []);
    ...
    $db_refs = normalize_string_list(is_array($relevant) ? ($relevant['db'] ?? []) : []);
    $invariant_refs = normalize_string_list(is_array($relevant) ? ($relevant['invariants'] ?? []) : []);

    $lines = [
        '# Task Pack ' . $task_id . ': ' . ($title !== '' ? $title : 'Task'),
        '',
        '## Goal',
        '- ' . ($goal !== '' ? $goal : '...'),
        '',
    ...
    $lines[] = '';
    $lines[] = '## Relevant Contracts';
    $lines[] = '- API: /contracts/api.yaml' . ($api_refs ? (' (endpoints: ' . implode(', ', $api_refs) . ')') : '');
    $lines[] = '- DB: /contracts/db.sql' . ($db_refs ? (' (tables: ' . implode(', ', $db_refs) . ')') : '');
    $lines[] = '- Invariants: /contracts/invariants.yaml' . ($invariant_refs ? (' (ids: ' . implode(', ', $invariant_refs) . ')') : '');
    $lines[] = '';
    ...
    $lines[] = format_pack_list_line('Dependencies', $dependencies);
    $lines[] = format_pack_list_line('Constraints', $constraints);
    $lines[] = format_pack_list_line('File allowlist', $file_allowlist);
    $lines[] = format_pack_list_line('Command allowlist', $command_allowlist);
    $lines[] = '';
...
function format_result_pack(string $task_id): string
{
    $lines = [
        '# Result Pack ' . $task_id,
        '',
...
function ensure_task_pack_files(
    string $repo_root,
    int $job_id,
    int $step_index,
    array $step
): array {
    $task_id = task_pack_id($job_id, $step_index);
    $task_pack = task_pack_path($job_id, $step_index);
    $result_pack = result_pack_path($job_id, $step_index);
    $created = [];

    $task_abs = safe_repo_path($repo_root, $task_pack);
    if (!file_exists($task_abs)) {
        $result = write_file_tool($repo_root, $task_pack, format_task_pack($task_id, $step));
        if (empty($result['error'])) {
            $created[] = $task_pack;
        }
    }

    $result_abs = safe_repo_path($repo_root, $result_pack);
    if (!file_exists($result_abs)) {
        $result = write_file_tool($repo_root, $result_pack, format_result_pack($task_id));
        if (empty($result['error'])) {
            $created[] = $result_pack;
        }
    }
    ...
    return [
        ...
        'result_pack_path' => $result_pack,
        'created' => $created,
    ];
}

function read_task_pack_content(string $repo_root, string $path, int $max_bytes = 8000): string
{
    $result = read_file_tool($repo_root, $path, $max_bytes);
    if (!empty($result['error'])) {
        return '';
...
function dept_head_prompt(
    ...
    $system = <<<SYS
You are the module-owner coordinator. Split the step into 1-4 module-owner subtasks.
Output JSON only:
{
  "subtasks": [
    {
...
function module_owner_prompt(
    ...
    string $architecture_summary,
    string $repo_memory,
    string $task_pack_path,
    string $task_pack_content,
    string $result_pack_path,
    bool $tests_enabled,
    string $test_command
): array {
    $system = <<<SYS
You are a module-owner coding agent operating on a local repo. Use the provided tools to inspect and modify files.
- Stay within the repo.
- Only edit owned paths listed in /docs/architecture.md. If ownership is unclear, update /docs/architecture.md first.
...
- Read the Task Pack and update /tasks/ACTIVE.md at start and end of the task.
- Write the Result Pack to the provided path using the template.
...
SYS;
    ...
    $criteria = $step['acceptance_criteria'] ?? [];
    $subtask_title = $subtask['title'] ?? 'Task';
    ...
    $file_allowlist_text = is_array($file_allowlist) ? implode("\n- ", $file_allowlist) : '';
    $command_allowlist_text = is_array($command_allowlist) ? implode("\n- ", $command_allowlist) : '';
    $task_pack_block = $task_pack_content !== ''
        ? ("\n\nTask Pack content:\n" . $task_pack_content)
        : '';
...
function validate_plan(array $plan): array
{
    ...
    foreach ($plan['steps'] as $step) {
        ...
        $relevant_contracts = [
            'api' => normalize_string_list($relevant['api'] ?? []),
            'db' => normalize_string_list($relevant['db'] ?? []),
            'invariants' => normalize_string_list($relevant['invariants'] ?? []),
        ];
        $steps[] = [
            'scope' => $scope,
            'title' => $title,
...
function process_job_ai(array $job, string $check_path): array
{
    ...
    append_log($job_id, 'module-owner: repo root ' . $repo_root);
    ...
    $needs_arch_doc = $architecture_doc === '';
    ...
    if (!$steps) {
        ...
        foreach ($plan_steps as $index => $step) {
            $task_info = ensure_task_pack_files($repo_root, $job_id, $index + 1, $step);
            if (!empty($task_info['created'])) {
                append_log($job_id, 'task-pack: created ' . implode(', ', $task_info['created']));
            }
        }
        $steps = list_steps($job_id);
    }
    ...
    $plan_steps_by_index = [];
    $plan_payload = read_json_file(plan_path($job_id));
    if (is_array($plan_payload) && isset($plan_payload['steps']) && is_array($plan_payload['steps'])) {
        foreach ($plan_payload['steps'] as $index => $step) {
            if (is_array($step)) {
                $plan_steps_by_index[$index + 1] = $step;
            }
        }
    }
    ...
    while (true) {
        ...
        $task_pack_path = $task_info['task_pack_path'];
        $result_pack_path = $task_info['result_pack_path'];
        $task_pack_content = read_task_pack_content($repo_root, $task_pack_path, 8000);
        if ($task_pack_content !== '') {
            $task_pack_content = truncate_text($task_pack_content, 6000);
        }
        $step_logger = static function (string $line) use ($job_id, $step_id) {
            append_log($job_id, $line);
            append_step_log($job_id, $step_id, $line);
        };
        $step_logger('module-owner: step ' . $step_index . ' (id ' . $step_id . ') - ' . $step['goal']);
        ...
        while (true) {
            ...
            $subtask_logger('module-owner: subtask ' . $subtask_row['subtask_index'] . ' (id ' . $sub_id . ') - ' . $subtask['title']);
            ...
            $subtask_logger('module-owner: subtask ' . $subtask_row['subtask_index'] . ' changed files +' . count($sub_changed));
        ...
        $step_logger('module-owner: step ' . $step_row['step_index'] . ' changed files +' . count($step_changed));
